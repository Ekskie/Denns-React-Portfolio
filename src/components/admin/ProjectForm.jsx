...
const ProjectForm = ({ onClose, onSaved }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    link: '#',
    techInput: '', // will split by comma
    statsJson: '{"ux": 90, "code": 85}' // Default JSON format
  });
    ...
    try {
      ...
      if (imageFile) {
        ...
        const { error: uploadError } = await supabase.storage
          .from('portfolio') // Ensure you created this bucket in Supabase
  ...
  return (
    <div className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4">
      <div className="bg-zinc-900 border border-cyan-500/30 p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto clip-polygon-card relative">
        ...
        <h2 className="text-xl font-bold text-cyan-400 mb-6 font-mono uppercase border-b border-zinc-800 pb-2">Initialize New Project</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 font-mono">
          ...
          <div>
            ...
            <div className="border border-dashed border-zinc-700 p-4 text-center hover:border-cyan-500 transition-colors cursor-pointer relative">
               <input type="file" accept="image/*" onChange={handleImageChange} className="absolute inset-0 opacity-0 cursor-pointer" />
               <div className="flex flex-col items-center gap-2 text-zinc-500">
                  <Upload size={20} />
                  <span className="text-xs">{imageFile ? imageFile.name : "Drag or Click to Upload"}</span>
               </div>
            </div>
          </div>
          ...
          <div>
             ...
             <textarea rows="2" className="w-full bg-black border border-zinc-700 p-2 text-green-400 font-mono text-xs focus:border-cyan-500 outline-none"
          ...
          <button disabled={loading} type="submit" className="w-full bg-cyan-600 hover:bg-cyan-500 text-black font-bold py-3 mt-4 flex justify-center items-center gap-2 uppercase tracking-widest clip-polygon">
            {loading ? <Loader2 className="animate-spin" /> : <><Save size={18} /> Compile & Save</>}
          </button>
        </form>
      </div>
    </div>
